    ...
    <section className="card p-4 sm:p-6">
      ...
      <ul className="divide-y divide-border">
        {blocks.filter(Boolean).map((b) => {
          if (!b) return null;
          return (
            <li key={b.hash} className="py-2.5 sm:py-3 flex items-center gap-2.5 sm:gap-3">
              <Link
                href={`/block/${b.number?.toString()}?net=${network}`}
                className="grid place-items-center w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-surface-2 text-accent font-display text-xs sm:text-base shrink-0 hover:bg-accent hover:text-accent-fg transition-colors"
                aria-label={`Block ${b.number?.toString()}`}
              >
                Bk
              </Link>
              <div className="flex-1 min-w-0">
                <div className="text-[13px] sm:text-sm font-bold text-text">
                  <Link
                    href={`/block/${b.number?.toString()}?net=${network}`}
                ...
                <div className="text-[11px] sm:text-xs text-text-muted truncate">
                  {b.transactions.length} tx · <HashLink hash={b.miner} network={network} type="block" />
                </div>
              </div>
              <div className="text-right">
                <div className="text-[11px] sm:text-xs text-text-muted whitespace-nowrap">
